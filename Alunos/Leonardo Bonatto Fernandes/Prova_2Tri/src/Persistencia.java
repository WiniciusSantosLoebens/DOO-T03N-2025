...
public class Persistencia {
    ...
    public static void inicializarSistema() {
        ...
        if (!new File(PASTA_USUARIOS + "/Lucas.json").exists()) {
            Usuario lucas = new Usuario("Lucas");
            Serie s1 = new Serie("Sherlock", "Inglês", List.of("Crime", "Drama", "Mistério"), "Encerrada", "2010-07-25", "2017-01-15", "BBC One");
            Serie s2 = new Serie("Dark", "Alemão", List.of("Ficção", "Suspense", "Drama"), "Encerrada", "2017-12-01", "2020-06-27", "Netflix");
            lucas.adicionar(lucas.getFavoritos(), s1);
            lucas.adicionar(lucas.getAssistidas(), s2);
            salvar(lucas);
        }

        if (!new File(PASTA_USUARIOS + "/Ana.json").exists()) {
            Usuario ana = new Usuario("Ana");
            Serie s1 = new Serie("The Office", "Inglês", List.of("Comédia"), "Encerrada", "2005-03-24", "2013-05-16", "NBC");
            Serie s2 = new Serie("The Mandalorian", "Inglês", List.of("Ação", "Aventura", "Ficção"), "Em andamento", "2019-11-12", "Ainda em exibição", "Disney+");
            ana.adicionar(ana.getFavoritos(), s2);
            ana.adicionar(ana.getParaAssistir(), s1);
            salvar(ana);
        }
    }
